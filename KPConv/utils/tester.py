...
class ModelTester:
    ...
    def regression_test(self, net, test_loader, config, num_votes=100, debug=False, chosen_log=''):
        ...
        # Initiate global prediction over test clouds
        self.test_preds = np.zeros((test_loader.dataset.num_models))
        self.test_counts = np.zeros((test_loader.dataset.num_models))
        ...
        while np.min(self.test_counts) < num_votes:
            ...
            for batch in test_loader:
                ...
                if (t[-1] - last_display) > 1.0:
                    ...
                    message = 'Test vote {:.0f} : {:.1f}% (timings : {:4.2f} {:4.2f})'
                    print(message.format(np.min(self.test_counts),
                                         100 * np.hstack(obj_inds).shape[0] / test_loader.dataset.epoch_n,
                                         1000 * (mean_dt[0]),
                                         1000 * (mean_dt[1])))
            # Stack all validation predictions
            ...
            print(Counter(self.test_counts.tolist()))
            self.test_preds[obj_inds] += (preds - self.test_preds[obj_inds]) / (self.test_counts[obj_inds])

            # Save/Display temporary results
            # ******************************

            test_labels = np.array(test_loader.dataset.input_labels)

            rss = np.sum((self.test_preds - test_labels) ** 2)
            tss = np.sum((test_labels - np.mean(test_labels)) ** 2)
            print(rss, tss)
            r2_score = 1 - rss / tss

            mae_value = np.mean(np.abs(self.test_preds - test_labels))

            print('Accuracies : val = {:.1f}%'.format(r2_score * 100))
            print('Mean absolute error : val = {:.4f}'.format(mae_value))

        results = []
        input_infos = test_loader.dataset.input_infos
        for i in range(self.test_preds.shape[0]):
            result = {
                'date': input_infos[i][1],
                'chicken_id': input_infos[i][2],
                'frame_file': input_infos[i][3],
                'pred_weight': self.test_preds[i],
            }
            results.append(result)
        
        with open(f'tempResults/results_{chosen_log}.json', 'w') as f:
            json.dump(results, f)
