...
# tensorboardX to track and visualize results
...
class ModelTrainer:
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        # Initialize tensorboardX writer. Results saved in tensorboardX/
        writer = SummaryWriter(f'tensorboardX/{config.saving_path.split("/")[1].split("_")[1]}')
        ...
        for epoch in range(config.max_epoch):
            ...
            for batch in training_loader:
                ...
                # tensorboardX log
                writer.add_scalar('acc', acc, self.step)
            ...
            # Validation
            net.eval()
            r2 = self.validation(net, val_loader, config)
            net.train()

            # Log results to tensorboardX
            writer.add_scalar('r2', r2, self.step)

        # End tensorboardX run
        writer.close()
    ...
    def validation(self, net, val_loader, config: Config):
        ...
        elif config.dataset_task == 'regression':
            return self.linear_regression_validation(net, val_loader, config)
        else:
            raise ValueError('No validation method implemented for this network type')

    def linear_regression_validation(self, net, val_loader, config):
        """
        ...
        if not hasattr(self, 'val_probs'):
            self.val_preds = np.zeros((val_loader.dataset.num_models))
        ...
        preds = preds.squeeze()
        self.val_preds[obj_inds] = val_smooth * self.val_preds[obj_inds] + (1-val_smooth) * preds

        rss = np.sum((preds - targets) ** 2)
        tss = np.sum((targets - np.mean(targets)) ** 2)
        r2_score_1 = 1 - rss / tss

        all_targets = val_loader.dataset.input_labels
        rss = np.sum((self.val_preds - all_targets) ** 2)
        tss = np.sum((all_targets - np.mean(all_targets)) ** 2)
        r2_score_2 = 1 - rss / tss

        print('Accuracies : val = {:.4f}% / vote = {:.4f}%'.format(r2_score_1 * 100, r2_score_2 * 100))
